...
# ── MANUAL STOCK ADDITION (ADMIN - MULTILINE) ──────────────────────────────
async def cmd_addstock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    
    # Replace with your actual Telegram Admin ID(s)
    ADMIN_IDS = [8695363498] 
    if uid not in ADMIN_IDS:
        await update.message.reply_text("⛔ You are not authorized to use this command.")
        return

    # Check if the admin replied to a message or provided arguments
    reply_message = update.message.reply_to_message
    if not reply_message:
        await update.message.reply_text(
            "⚠️ <b>How to use:</b>\n"
            "1. Send your account details block as a message.\n"
            "2. Reply to that message with <code>/addstock 25</code> or <code>/addstock 5</code>",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    args = context.args
    if not args:
        await update.message.reply_text("⚠️ Please specify the tier! Example: <code>/addstock 25</code>", parse_mode=ParseMode.HTML)
        return

    tier = args[0]
    if tier == "25":
        filename = "stock_25.txt"
    elif tier == "5":
        filename = "stock_5.txt"
    else:
        await update.message.reply_text("⚠️ Invalid tier! Use <b>5</b> or <b>25</b>.", parse_mode=ParseMode.HTML)
        return

    # Extract the full text of the replied message
    account_block = reply_message.text or reply_message.caption
    if not account_block:
        await update.message.reply_text("❌ The replied message has no text content.")
        return

    try:
        # Append the entire multi-line block followed by a clean separator
        with open(filename, "a", encoding="utf-8") as f:
            f.write(account_block.strip() + "\n\n" + "="*50 + "\n\n")
            
        await update.message.reply_text(
            f"✅ <b>Successfully added multi-line stock to ₱{tier} Tier!</b>",
            parse_mode=ParseMode.HTML
        )
    except Exception as e:
        await update.message.reply_text(f"❌ Error saving stock: {e}")
